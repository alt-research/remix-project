...
const fetchContractFromEtherscan = (plugin, network, contractAddress, targetPath, shouldSetFile = true, key) => (0, tslib_1.__awaiter)(void 0, void 0, void 0, function* () {
    let data;
    const compilationTargets = {};
    let etherscanKey;
    if (!key)
        etherscanKey = yield plugin.call('config', 'getAppParameter', 'etherscan-access-token');
    else
        etherscanKey = key;
    if (etherscanKey) {
        const endpoint = network.id == 1 ? 'api.etherscan.io' : 'api-' + network.name + '.etherscan.io';
        try {
            data = yield fetch('https://' + endpoint + '/api?module=contract&action=getsourcecode&address=' + contractAddress + '&apikey=' + etherscanKey);
            data = yield data.json();
            // etherscan api doc https://docs.etherscan.io/api-endpoints/contracts
            if (data.message === 'OK' && data.status === "1") {
                if (data.result.length) {
                    if (data.result[0].SourceCode === '')
                        throw new Error(`contract not verified on Etherscan ${network.name} network`);
    ...
    if (typeof data.result[0].SourceCode === 'string') {
        ...
        if (shouldSetFile)
            yield plugin.call('fileManager', 'setFile', fileName, data.result[0].SourceCode);
        ...
        for (let [file, source] of Object.entries(sources)) { // eslint-disable-line
            ...
            if (yield plugin.call('contentImport', 'isExternalUrl', file)) {
                ...
                if (shouldSetFile)
                    yield plugin.call('fileManager', 'setFile', path, content);
...
//# sourceMappingURL=fetch-etherscan.js.map